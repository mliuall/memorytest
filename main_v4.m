...
function main()
block_initial = ones(32,32); %Initialization
...
%3/20: Got the test case to work: displays the wanted sequence for 2
%seconds
figure('Name','Test','NumberTitle','off'); 
    ...
    for ii = 1:3
        ...
        subplot(3,3,k)
            if mod(k,2) == 1
                img = image(block_initial);
            else
                img = image(block_initial+256);
            end
            set(img,'HitTest','off'); axis off;
            k = k+1;
        end
